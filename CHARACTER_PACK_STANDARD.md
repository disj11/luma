...
현재 번들 캐릭터는 `Sources/AIPet/Resources/Pets/LunaSera`에 들어 있다.

앱에서 캐릭터를 추가하는 방법:

1. `manifest.json`이 들어 있는 캐릭터 폴더를 준비한다.
2. 메뉴바의 `Characters`를 연다.
3. `Add Character...`를 누르고 캐릭터 폴더를 선택한다.
4. 앱은 해당 폴더를 `~/Library/Application Support/Luma/Characters/<character-id>`로 복사하고 즉시 선택한다.

처음 선택 가능한 기본 캐릭터는 내장된 `루나 세라` 하나다.
